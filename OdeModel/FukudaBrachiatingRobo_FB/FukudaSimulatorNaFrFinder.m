function FukudaSimulatorNaFrFinder()
...
while(Tol>epsilone)
    Tn=2*pi/ww(end);
    [T,Y] = ode15s(@(t,X)SirDyn(t,X,g,L1,L2,mp1,mp2,ww(end)),[0 Tn/4],InitState,OdeOpt);
    ww(end+1)=ww(end)+Kstep*Y(end,1);
    Tol=abs(Y(end,1));
end
...
Torque=TorCal(Time,Y,g,L1,L2,mp1,mp2,ww(end));
Torque=Torque(1:end-1);
