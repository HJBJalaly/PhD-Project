function AnimBot(T,Y,Pin1Pos,L1,L2,d0)
...
%         
        ...
        Tracker=[];
        for i=1:length(T)-1
            Link1UpPos=[Pin1Pos(1)  , Pin1Pos(2)];
            Link1DnPos=[Pin1Pos(1)+(L1)*sin(Y(i,1))   , Pin1Pos(2)-(L1)*cos(Y(i,1))];
            Pin2Pos   = Link1DnPos;
            Link2DnPos= Pin2Pos;
            Link2UpPos=[Pin2Pos(1)+(L2)*sin(Y(i,1)+Y(i,2))  , Pin2Pos(2)-(L2)*cos(Y(i,1)+Y(i,2))];
            Pin3Pos=Link2UpPos;
            Tracker=[Tracker;Pin3Pos];
            ...
            set(M4Hndl,'XData',  Pin3Pos(1)                  , 'YData', Pin3Pos(2));
