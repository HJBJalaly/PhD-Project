...
%%
[Torque_X0,Q_X0,D1Q_X0,D2Q_X0,Nothing,Nothing,IntU2_X0,IntUdq_X0,IntAbsUdq_X0,Nothing,CostSlope_X0,Nothing,Nothing,RMSError_X0]=...
                        ShowTime(Initial,Time,Tres,Degree,Weight,Landa,[],[],[],XEF,YEF,m,L,g,[],'DntShow','2Cycle','CostA','Initial');
[Torque_Opt,Q_Opt,D1Q_Opt,D2Q_Opt,Nothing,Nothing,IntU2_Opt,IntUdq_Opt,IntAbsUdq_Opt,Nothing,CostSlope_Opt,Nothing,Nothing,RMSError_Opt]=...
...
TauMax=max(tauShift);
tauShiftScale=(tauShift)/TauMax;

Range=[90:95]; % range of fiting

NewFeasible=1;
PosFlag=1;  % 0 for first, 1 for end
...
if(NewFeasible==0)
    tauShiftScaleVar=tauShiftScale;
end
...
figure
    ...
    subplot(2,1,2)
    plot(ThetaShiftScale(1:end-1),diff(tauShiftScale)/1./diff(ThetaShiftScale))
    xlabel('ThetaShiftScale')
    ...
    hold off
    hold all
    plot(ThetaShiftScale(1:end-1),diff(tauShiftScaleVar)/1./diff(ThetaShiftScale))
    hold off
    

if(~PosFlag)
    tauDiff=tauShiftScale(1:Range(1))-tauShiftScaleVar(1:Range(1)); % for first
    ThetaDiff=ThetaShiftScale(1:Range(1));   % for first
else
    tauDiff=tauShiftScale(Range(2):end)-tauShiftScaleVar(Range(2):end);   % for end
    ThetaDiff=ThetaShiftScale(Range(2):end);   % for end
end
figure
...
CostParam=@(Param)GA_CostParamNonLinearSpring(Param,ThetaStepscale,ThetaShiftScale,tauShiftScaleVar);
...
NonLinearSpring(ThetaStepscale,ThetaShiftScale,tauDiff,k,R,q0,.1)
% NonLinearSpring(ThetaStep,ThetaS,tauShift,k,R,q0,.1)
