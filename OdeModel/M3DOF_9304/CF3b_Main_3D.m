...
% % Ellipose motion
xefr=A*cos(2*pi*f*time)+0;
yefr=A/2*sin(2*pi*f*time)+1.5;
Dxefr=-(2*pi*f)*A*sin(2*pi*f*time);
Dyefr= (2*pi*f)*A/2*cos(2*pi*f*time);
D2xefr=-(2*pi*f)^2*A*cos(2*pi*f*time);
D2yefr=-(2*pi*f)^2*A/2*sin(2*pi*f*time);
q1r=deg2rad(-60);
q2r=deg2rad(48.031);
q3r=deg2rad(-48.031);
...
posr=[xefr;yefr];
Dposr=[Dxefr;Dyefr];
D2posr=[D2xefr;D2yefr];


% Joint Trajectories
for tt=1:length(time)-1
    lPosr=L*[cos(q1r(tt))+cos(q1r(tt)+q2r(tt))+cos(q1r(tt)+q2r(tt)+q3r(tt));
            sin(q1r(tt))+sin(q1r(tt)+q2r(tt))+sin(q1r(tt)+q2r(tt)+q3r(tt))];

    cPos=posr(:,tt);
    dx=cPosr-lPosr;

    JJ=L*[-sin(q1r(tt))-sin(q1r(tt)+q2r(tt))-sin(q1r(tt)+q2r(tt)+q3r(tt)), -sin(q1r(tt)+q2r(tt))-sin(q1r(tt)+q2r(tt)+q3r(tt)), -sin(q1r(tt)+q2r(tt)+q3r(tt));
           cos(q1r(tt))+cos(q1r(tt)+q2r(tt))+cos(q1r(tt)+q2r(tt)+q3r(tt)),  cos(q1r(tt)+q2r(tt))+cos(q1r(tt)+q2r(tt)+q3r(tt)),  cos(q1r(tt)+q2r(tt)+q3r(tt))]; 

    dq=JJ'*(JJ*JJ')^-1*dx;
    q1r(tt+1)=q1r(tt)+dq(1);
    q2r(tt+1)=q2r(tt)+dq(2);
    q3r(tt+1)=q3r(tt)+dq(3);
end

rPosValr=L*[cos(q1r)+cos(q1r+q2r)+cos(q1r+q2r+q3r);
            sin(q1r)+sin(q1r+q2r)+sin(q1r+q2r+q3r)];
...
%     legend('Desired Torque','PassiveTorque')
    ...
    subplot(3,3,3)
    plot(Qhat1,TorqueDesQ1+TorqueDesQ2-TorqueMonoOptimal(1,:)-TorqueMonoOptimal(2,:)-TorqueBicepsOptimal(2,:),'linewidth',2,'linestyle','-','color','b')
    hold on
    plot(Qhat1,2*TorqueBicepsOptimal(1,:),'linewidth',2,'linestyle','-.','color','r')
    hold off
    grid on
    xlabel('q_1+q_2')
    ylabel('u_r_1+u_r_2-u_m_1-u_m_2-u_b_2')
%     legend('Desired Torque','PassiveTorque')

    subplot(3,3,6)
    plot(Qhat2,TorqueDesQ2+TorqueDesQ3-TorqueMonoOptimal(2,:)-TorqueMonoOptimal(3,:)-TorqueBicepsOptimal(1,:),'linewidth',2,'linestyle','-','color','b')
    hold on
    plot(Qhat2,2*TorqueBicepsOptimal(2,:),'linewidth',2,'linestyle','-.','color','r')
    hold off
    grid on
    xlabel('q_2+q_3')
    ylabel('u_r_2+u_r_3-u_m_2-u_m_3-u_b_1')
