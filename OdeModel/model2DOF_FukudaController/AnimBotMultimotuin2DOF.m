function AnimBotMultimotuin2DOF(T,Y,XY,L)
...
%         
        ...
        Tracker=[];
        for i=1:length(T)-1
            if(T(i)==XY(MotionNo+1,3))
                MotionNo=MotionNo+1;
                MFC1=get(P1Hndl,'MarkerFaceColor');
                MEC1=get(P1Hndl,'MarkerEdgeColor');
                MFC2=get(M4Hndl,'MarkerFaceColor');
                MEC2=get(M4Hndl,'MarkerEdgeColor');
                set(P1Hndl,'MarkerFaceColor',MFC2);
                set(P1Hndl,'MarkerEdgeColor',MEC2);
                set(M4Hndl,'MarkerFaceColor',MFC1);
                set(M4Hndl,'MarkerEdgeColor',MEC1);               
            end
            
            Link1UpPos=[XY(MotionNo,1)     , XY(MotionNo,2)];
            Link1DnPos=[XY(MotionNo,1)+(L)*sin(Y(i,1))   , XY(MotionNo,2)-(L)*cos(Y(i,1))];
            Link2DnPos= Link1DnPos;
            Pin2Pos   = Link1DnPos;
            Link2UpPos=[Link1DnPos(1)+(L)*sin(Y(i,2)-Y(i,1))  , Link1DnPos(2)+(L)*cos(Y(i,2)-Y(i,1))];
            Pin4Pos   =[Link1DnPos(1)+(L)*sin(Y(i,2)-Y(i,1))  , Link1DnPos(2)+(L)*cos(Y(i,2)-Y(i,1))];
            Tracker=[Tracker;Pin4Pos];
            
            set(GRHndl,'XData', [XLimits(1) XLimits(2)]      , 'YData', [0 0]);
            set(P1Hndl,'XData', XY(MotionNo,1)               , 'YData', XY(MotionNo,2));
