function w_nat=OdeSirFukudaControllerFindNaFr4DOF(Space,show)
...
[rr_rest,theta_rest] = InitCondinRest4DOF(Space,L,d0,mL1,mp3,K1,g);
...
while (Tol>epsilone)
    ww(end+1)=ww(end)+Kstep*Y(end,2); % theta1
    Tn=2*pi/ww(end);
    [T,Y] = ode15s(@(t,X)SirDyn(t,X,g,d0,L,mL1,mL2,mp3,mp4,K1,K2,B1s,B2s,B1t,B2t,ww(end)),[0 Tn/4],InitState,OdeOpt);
    Tol=abs(Y(end,2));
...
%     fitresult
% 
%     [xData, yData] = prepareCurveData(Time, theta_vir_es);
%     [fitresult, gof] = fit( xData, yData, 'sin1' );
%     fitresult
   
    Torque=TorCal(Time,Y,g,d0,L,mL1,mL2,mp3,mp4,K1,K2,B1s,B2s,B1t,B2t,w_nat);
...
GAMMA1=0;
NN=MM^-1;
GAMMA2=(Dqh * NN(:,4))^-1 * ( -ww^2*h + Dqh*NN*(CC*D1q+GG + [B1s B1t B2s B2t]'.*D1q));
% r_vir=sqrt((L+d0+r1).^2+(L+d0+r2).^2-2*(L+d0+r1).*(L+d0+r2).*cos(((theta2))));
...
for i=1:length(Time)
    ...
    % Input: compute torque
    NN=MM^-1;
    Torque(i)=(Dqh * NN(:,4))^-1 * ( -ww^2*h + Dqh*NN*(CC*D1q+GG - [B1s B1t B2s B2t]'.*D1q) );
