function hpol = Mypolar(varargin)
    ...
    if ~hold_state
        ...
        for i = 1 : 1 : rmax
            if(~mod(i,5))
                text((i + rinc / 20) * c82, (i + rinc / 20) * s82, ...
                    ['    ' num2str(i) ' cm'], 'VerticalAlignment', 'bottom', ...
                    'HandleVisibility', 'off', 'Parent', cax,'FontSize',11,...
                    'FontName','mwa_cmb10','FontWeight','bold');
                hhh = line(xunit * i, yunit * i, 'LineStyle', '-.', 'Color', tc, 'LineWidth', 1.5, ...
                'HandleVisibility', 'off', 'Parent', cax);
        ...
        rt = 1.2 * rmax;
        th2 = (0 : 12) * 2 * pi / 12;
        for i = 1 : length(th2)-2
            text(rt * cos(th2(i)), rt * sin(th2(i)), int2str((i-1) * 30),...
